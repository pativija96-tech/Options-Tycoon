...
@router.post("/run-eod")
async def run_eod_resolution():
    """Resolve all open trades using actual NIFTY closing price."""
    import asyncio
    from concurrent.futures import ThreadPoolExecutor
    ...
    try:
        ...
        with ThreadPoolExecutor() as pool:
            result = await asyncio.wait_for(
                loop.run_in_executor(pool, _resolve_open_trades),
                timeout=60
            )
        return result
    except Exception as e:
...
def _resolve_open_trades() -> dict:
    """Resolve all open trades in DB using real NIFTY close from yfinance."""
    ...
    try:
        ...
        if data is None or len(data) < 1:
            return {"success": False, "error": "Could not fetch NIFTY close"}
        close_col = data["Close"]
        if hasattr(close_col, "columns"):
            close_col = close_col.iloc[:, 0]
        nifty_close = float(close_col.iloc[-1])
    except Exception as e:
        return {"success": False, "error": f"NIFTY fetch failed: {str(e)[:100]}"}
    
    # Resolve open trades
    conn = get_connection()
    try:
        open_trades = conn.execute(
            "SELECT id, direction, strategy, legs, entry_cost, max_loss, max_profit, projected_open, width FROM live_trades WHERE status = 'open'"
        ).fetchall()
        
        if not open_trades:
            return {"success": True, "resolved": 0, "nifty_close": nifty_close, "message": "No open trades to resolve"}
        ...
        for trade in open_trades:
            ...
            entry_cost = trade_dict.get("entry_cost", 0) or 0
            width = trade_dict.get("width", 200) or 200
            
            # Calculate P&L based on strategy type and actual NIFTY movement
            if "bull_call" in strategy:
                # Bull call spread profits when NIFTY goes UP
                legs_data = trade_dict.get("legs", "[]")
                try:
                    import json as jm
                    legs = jm.loads(legs_data) if isinstance(legs_data, str) else legs_data
                    long_strike = legs[0].get("strike", 0) if legs else projected_open
                except:
                    long_strike = projected_open
                
                if nifty_close > long_strike:
                    intrinsic = min(nifty_close - long_strike, width)
                    pnl = min((intrinsic * 65) - abs(entry_cost), max_profit)
                else:
                    pnl = -abs(entry_cost) if entry_cost > 0 else -max_loss
                ...
                # Iron condor profits when NIFTY stays in range
                legs_data = trade_dict.get("legs", "[]")
                try:
                    import json as jm
                    legs = jm.loads(legs_data) if isinstance(legs_data, str) else legs_data
                ...
                if short_put <= nifty_close <= short_call:
                    # NIFTY stayed in range — max profit
                    pnl = max_profit
                elif nifty_close > short_call:
                    breach = nifty_close - short_call
                    pnl = max(max_profit - (breach * 65), -max_loss)
                else:
                    breach = short_put - nifty_close
                    pnl = max(max_profit - (breach * 65), -max_loss)
            else:
                # Generic directional
                if direction == "bullish" and nifty_move > 0:
                    ...
                    pnl = -max_loss if max_loss > 0 else -abs(nifty_move) * 65 * 0.2
            
            pnl = round(pnl)
            status = "win" if pnl > 0 else "loss"
            
            conn.execute(
                "UPDATE live_trades SET status = ?, pnl = ?, nifty_close = ?, exit_reason = 'eod_resolution', resolved_at = ? WHERE id = ?",
                (status, pnl, nifty_close, datetime.now().isoformat(), trade_dict["id"])
            )
            resolved_count += 1
